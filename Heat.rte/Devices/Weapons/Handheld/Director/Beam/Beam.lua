...
function Update(self)
	...
	if self.cast then
		...
		if travelMagnitude > 5 then
			...
			for i = 0, maxi do
				--PrimitiveMan:DrawCirclePrimitive(endPos + travel / maxi * i, 2 + i / maxi * 3, math.floor(154 + 3 / maxi * i));
	...
	if (self.ray > -1 or self.hits > 0) and self.hits < 1 then
		...
		smoke.Pos = self.hitPos - Vector(2, 0):RadRotate(self.RotAngle);
		...
		if self.hits == 1 then
			...
			if math.random(1,3) < 2 then
				self.beamHitSound:Play(self.hitPos);
			end
